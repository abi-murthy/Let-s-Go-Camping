...
describe("ParkCard", () => {
    test("renders without crashing", () => {
        render(<ParkCard park={mockPark.park} />);
        expect(screen.getByText("Mock Park")).toBeInTheDocument();
    });

    test("toggle favorite status", async () => {
        render(<ParkCard park={mockPark.park} />);
        const favoriteButton = screen.getByTestId(/favorite-button/i);
        await act(async () => {
            await userEvent.click(favoriteButton);
        });


        await waitFor(() =>expect(screen.getByTestId(/confirm-add-button/i)).toBeInTheDocument());
        const confirmFavoriteButton = screen.getByTestId(/confirm-add-button/i)

        await act(async ()=> {
            await userEvent.click(confirmFavoriteButton)
        })
